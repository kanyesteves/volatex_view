<template>
  <Dialog v-model:visible="visible" modal header="Aviso de confirmação" :style="{ width: '25rem' }">
    ...
    <div :class="$style.space_bottons">
      ...
      <Button :style="{ 'margin-left': '1rem' }" type="button" severity="danger" label="Remover" @click="onRemoveArticle"></Button>
    </div>
  </Dialog>
</template>
...
const onRemoveArticle = () => {
  articleService.remove(props.article.id).then((response) => {
